    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(".addInsumoBtnModal").click(function (e) {
                ...
                var idNombreUnidad = insumoPorConfigurar.unidad.nombre;//cambiar
    ...
    $.ajax({
        ...
        success: function (result) {    
            ...
            var insumoConfigurado = {                
                ...
                unidadBaseId: insumoPorConfigurar.idUnidadBase,//cambiar
...
function refreshDataTableInsumos() {
    ...
    $table.html("");    
    $.each(Receta.insumos, function (key, value) {//cambiar
    ...
    $.each(presentaciones, function (index, value) {
        if (!value.insumo["deshabilitado"]) {
            ...
            $("#row").append(
                ...
                '<div class="singleCard" id="' + value.insumo["id"] + '-' + value["id"] + '">' +
                '<div class="cardSobrePuesta"></div>' +
                '<div class="cardInsumo">' +
                '<div class="imgText">' +
                '<div class="img">' +
                '<img src="/images/insumo.svg">' +
                '</div>' +
...
$("#txtBuscar").keyup(function () {
    ...
    $.each(presentaciones, function (index, valor) {
        if (!valor.insumo["deshabilitado"]) {
            ...
            $("#row").append(
                '<div class="col-md-2 col-sm-12 offset-sm-0 divCard">' +
                '<div class="singleCard" id="' + valor.insumo["id"] + '-' + valor["id"] +'">' +
                        '<div class="cardSobrePuesta"></div>' +
                        '<div class="cardInsumo">' +
                            '<div class="imgText">' +
                                '<div class="img">' +
            ...
            $("#btnRegistrarCompras").click(function (event) {
                ...
                $.each(targets, function (index, valor) {
                    ...
                    if (parseInt(cantidad) < 1 || cantidad.length < 1 ||
                        parseInt(precio) < 1 || precio.length < 1) {
                        hayNegativos = true;
                    }
                }); 
                ...
                if (hayNegativos) {
                    ...
                    $.each(targets, function (index, valor) {
                        ...
                        var stockTotal = (parseInt(presentacionUnidad) * parseInt(cantidad));
                        $.ajax({
...
$(document).on('click','.insumoSeleccionado',function () {//cambiar todo
                ...
                console.log(data);
                    ...
                    for (var i = 0; i < data[1].length; i++) {
                        ...
                        var insumoConfigurado = {
                            ...
                            unidadBaseId: data[0].insumosReceta[i].IdUnidad,//cambiar
...
$("#editRecetaForm").submit(function (e) {
    if (!$("#editRecetaForm").valid()) {
        $("#editRecetaForm.input-validation-error").focus();
        return;
    }
    e.preventDefault();
    var insumosParam = [];
    ...
    $.each(Receta.insumos, function (i, value) {
        var viewInsumo = {
            IdInsumo: value.id,
            IdUnidad: value.unidadBaseId,//cambiar 
            PesoNeto: value.pesoNeto
        }
        insumosParam.push(viewInsumo);
    });
...
$("#form_create_insumo").submit(function (e) {
    if (!validarCategoria()) {
        e.preventDefault();
    }
});
...
$(document).on('click', '.insumoInventario', function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#imagenInsumo").attr("src", "/images/insumo.svg");
